...
public class Parqueadero {
	...
	public boolean estaLibre(Puesto puesto) {
		...
		return puesto.isEstado();
	}

	public boolean esCompatible(Vehiculo vehiculo, Puesto puesto) {

		return vehiculo.getTipoVehiculo().getNombre().equalsIgnoreCase(puesto.getTipoPuestoVehiculo());
	...
	public Reserva registrarSalida(Reserva reserva, Vehiculo vehiculo) {
		...
		reserva.setValorTotal(
				this.calcularValor(vehiculo.getTipoVehiculo(), vehiculo.getCilindraje(), fechaIngreso, fechaRetiro));
		return reserva;
	}
	...
	public float calcularValor(TipoVehiculo tipoVehiculo, short cilindraje, Calendar fechaIngreso,
		...
		if (tiempoParqueo < 9) {
			...
			if (tiempoParqueo == 0) {
				valor += TablaPrecio.devolverValorHora(tipoVehiculo) * 1;
			} else {
